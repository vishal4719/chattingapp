...
import {
  ...
  shouldShowBrowserNotificationBanner,
...
export function BrowserNotificationBanner() {
  ...
  useEffect(() => {
    if (isNativeApp() || !isBrowserNotificationSupported()) return;

    function refreshBanner() {
      setVisible(shouldShowBrowserNotificationBanner());
    }
  ...
  async function handleEnable() {
    ...
    try {
      ...
      if (ok) {
        ...
        setError("Could not enable browser notifications. Check site permissions.");
      }
    } catch {
  ...
  function handleDismiss() {
    localStorage.setItem("browser-notifications-dismissed", "1");
    setVisible(false);
  }
    ...
    <div className="shrink-0 px-3 py-1.5 bg-[var(--wa-green)]/15 border-b border-[var(--wa-green)]/30 flex items-center gap-2 text-xs md:text-sm">
      ...
      <div className="flex-1 min-w-0">
        <p className="text-[var(--wa-text)]">
          Enable browser notifications to get message alerts when this tab is in the background.
        </p>
...
export { isBrowserNotificationEnabled };
